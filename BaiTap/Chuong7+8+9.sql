...
--9.2. Hiển thị danh sách của nhân viên thuộc sự quản lý của người có tên là 1 biến được nhập từ bàn phím
SELECT EMPNO,ENAME,JOB,MGR,HIREDATE,NVL(SAL,0) AS SAL,NVL(COMM,0) AS COMM,DEPTNO
FROM EMP WHERE MGR = (SELECT EMPNO FROM EMP WHERE JOB='MANAGER' AND ENAME='CLARK');